		...
		<center>Ventas </center><br /><table width="845" border="1">
          <tr>
           ...
           <? // $sql2="select *, SUM(pe.conver_m3)AS Total from pesaje pe, mezclas mez, obra ob where pe.fecha_hora  BETWEEN '".$fecha_inicio."' AND '".$fecha_final."' and pe.obra=".$obra." and pe.obra=ob.id and pe.tipo_mezcla=mez.id GROUP BY pe.tipo_mezcla ";
			...
			$sql2="select *,SUM(dp.litros)AS Total from pesaje pe ,detalle_pesaje dp where pe.fecha_hora  BETWEEN '".$fecha_inicio."' AND '".$fecha_final."' and pe.nula!='si' and pe.id=dp.guia  group by dp.producto";
//			echo $sql2;
		...
		<center>
          ...
          <tr>
           ...
           <? 
			$igual=0;
			$sql23="select *,SUM(dp.cantidad)AS Total from materias pe ,ingreso_materias dp where dp.fecha_guia  BETWEEN '".$fecha_inicio."' AND '".$fecha_final."' and pe.id=dp.materia  group by dp.materia";
