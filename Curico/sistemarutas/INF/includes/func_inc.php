...
function lista_orden($formulario)
  {
	...
	/* $select2="select ingreso_orden where orden=".$num_orden;
	$result_sl = mysql_query($select2); 
		while($row2 = mysql_fetch_row($result_sl))
		{ 
			$cantidad =$row2[6];
		}
		
		if($cantidad>=$total && $cantidad<10)
		{
		 echo "ingrese Contraseña";
		}
	*/
	
	$sql="insert into detalle_orden_compra(id_orden_compra,cantidad,unidad,descripcion,precio_unitario,total) values ('$num_orden','$cantidad','$unidad','$producto','$valor_unitario','$preciototal')";
	$result = mysql_query($sql);
	$sql2 ="SELECT  * from orden_compra pre, detalle_orden_compra deta where pre.id=".$num_orden." and pre.id=deta.id_orden_compra ";
	$result2 = mysql_query($sql2); 
	$contenido="<tr><th width='34'>ID</th> <th width='296'>Nombre Producto</th> <th width='87'>Cantidad</th> <th width='78'>Valor Unitario</th> <th width='73'>Unidad</th> <th width='56'>Total</th> <th width='80'>Eliminar</th> </tr> ";
		...
		{ 
		$contenido.=" <tr>";
		$contenido.="<td >".$row[12]." </td>";
          $contenido.="<td height='58'>".$row[16]." </td>";
            $contenido.=" <td>".$row[14]."</td>";
           $contenido.="  <td> ".$row[15]."</td>";
            $contenido.=" <td>".$row[17]."</td>";
			$total_final=$row[14]*$row[17];
			$contenido.=" <td>".$total_final."</td>";
            $contenido.=" <td><input type='button' onclick='xajax_elim_product_orden(document.form.num_orden.value,".$row[12].")' value='Eliminar' class='formbutton2' ></td>";
         $contenido.="  </tr>";
          }     
    $respuesta->addAssign("listado_productos","innerHTML",$contenido);
...
function elim_product_guia($guia,$id)
  {
   include("conexion.php"); 
	...
	//$sql2 ="SELECT * from detalle_pesaje where guia=".$guia." ";
	$sql2 ="SELECT * from detalle_pesaje dp, mezclas ml where dp.guia=".$guia." and dp.producto=ml.id ";
	$result2 = mysql_query($sql2); 
		$contenido="<tr><th width='34'>ID</th> <th width='275'>Nombre Producto</th> <th width='66'>Cantidad</th> <th width='120'>Valor Unitario</th> <th width='97'>Total</th>  <th width='80'>Eliminar</th> </tr> ";
		while($row = mysql_fetch_row($result2))
		{ 
