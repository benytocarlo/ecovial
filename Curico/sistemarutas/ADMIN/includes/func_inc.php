...
function operacion_calculo($patente,$materia)
  {
	//print($patente);
	include("conexion.php"); 
    $respuesta = new xajaxResponse();
	$sql2="Select * from camiones where id=".$patente." ";
	...
	$sql22="Select * from mezclas where id=".$materia." ";
	...
	//print($sql22);	
	$totalsinpor	= ($capacidad-$tara) / $valor;
	/* porcentaje */
	$porcentaje =  number_format($totalaplicado*0.5/100 ,1);
	
	$total = $totalsinpor - $porcentaje;
	
	$kilo = $capacidad - $tara;
	$porcentaje2 =  number_format($kilo*0.5/100 ,1);
	$kilototal = $kilo - $porcentaje2;
	
	$totalround = number_format($total, 0, ",", "."); 
	$kiloround = number_format($kilototal, 0, ",", "."); 
	
	$val = explode(",",$totalround);

	if($val[1]<9 && $val[1]>5){ $decimal = 5; }
	if($val[1]=5) { $decimal=5; }
	if($val[1]>1 && $val[1]<5){ $decimal = 0; }
	
	//$totalaplicado = $val[0].",".$val[1];
	$totalaplicado = $val[0];
	$respuesta->addAssign("result","value",$totalaplicado);
	$respuesta->addAssign("kilo","value",$kiloround);
    return $respuesta;
  }
 /////////////////////////////////////////////////////////////////////////////////////////////// 


function buscar_guia($formulario)
  {
	include("conexion.php"); 
    $respuesta = new xajaxResponse();
	$n_guia = $formulario["n_guia"];
	$sql2="Select * from mezclas ml,  pesaje pe, detalle_pesaje dt, obra ob, camiones cam ,cliente cli where pe.cliente=cli.id and pe.id=".$n_guia." and dt.guia=pe.id and dt.producto=ml.id and pe.patente=cam.id  and pe.obra=ob.id and pe.tipo_guia='emulsion' ";
	$result = mysql_query($sql2); 
	    while($row = mysql_fetch_row($result))
		{
		 $emulsion=$row[2];
		 ...
		 $idmateria=$row[0];		 
			...
			$respuesta->addAssign("oc","value",$oc);
			$respuesta->addAssign("patente","value",$patente);
			$respuesta->addAssign("materia","value",$emulsion);
			$respuesta->addAssign("obra","value",$obra);
			$respuesta->addAssign("cliente","value",$cliente);
			$respuesta->addAssign("idmateria","value",$idmateria);
    return $respuesta;
  }
 /////////////////////////////////////////////////////////////////////////////////////////////// 
 ///////////////////////////////////////////////////////////////////////////////////////////////
...
function lista_orden($formulario)
  {
	...
	$result = mysql_query($sql);
	$sql3 ="SELECT  * from detalle_orden_compra where id_orden_compra=".$num_orden." ";
	...
	while($row = mysql_fetch_row($result2))
		...
		$contenido.="<td >".$row[0]." </td>";
          $contenido.="<td height='58'>".$row[4]." </td>";
            $contenido.=" <td>".$row[2]."</td>";
           $contenido.="  <td> ".$row[5]."</td>";
            $contenido.=" <td>".$row[3]."</td>";
			$total_final=$row[6];
			$contenido.=" <td>".$total_final."</td>";
            $contenido.=" <td><input type='button' onclick='xajax_elim_product_orden(document.form.num_orden.value,".$row[0].")' value='Eliminar' class='formbutton2' ></td>";
         $contenido.="  </tr>";
		 $total=$total+$total_final;
          }
	$iva=$total*0.19;
	$ivafinal= number_format($iva, 0, ".", "");  
	$bruto=$total+$iva;
	$brutofinal= number_format($bruto, 0, ".", "");  
	$respuesta->addAssign("total_neto","value",$total);
	$respuesta->addAssign("total_bruto","value",$brutofinal);		
	$respuesta->addAssign("iva","value",$ivafinal);	
    $respuesta->addAssign("listado_productos","innerHTML",$contenido);
...
function lista_guia($formulario)
 ...
 /////////////////////////////////////////////////////////////////////////////////////////////// 
 ///////////////////////////////////////////////////////////////////////////////////////////////
function lista_guia_edit($formulario)
  {
	 include("conexion.php"); 
    $respuesta = new xajaxResponse();
	$tipo_mezcla = $formulario["tipo_mezcla"];
	$n_litros = $formulario["n_litros"];
	$valor_mezcla = $formulario["valor_mezcla"];
	//$cantidad = $peso - $tara;
	$cantidad=$n_litros;
	if($tipo_mezcla=="" || $n_litros =="" || $valor_mezcla =="" )
	{
		$respuesta->addScript("alert('Hay Datos Vacios');");
		 return $respuesta;
	}
	$n_guia = $formulario["n_guia"];
...
function tara($patente)
  {
	...
	$result = mysql_query($query); 
	 ...
	 $query="Select * from registro_temperatura where patente=".$patente." order by fecha ASC";
	 $result = mysql_query($query); 
		while($row = mysql_fetch_row($result))
		{ 
		$temperatura= $row[3];
	...
	$respuesta->addAssign("temperatura","value",$temperatura);
    return $respuesta;
  }
///////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////
function calcularm3_aux($cliente,$orden_compra,$factor_mezcla,$pesobruto,$tara,$orde_compra)
  {
    $respuesta = new xajaxResponse();
	 include("conexion.php"); 
    ...
    if($orden_compra=="Eliga una Opcion")
	{
		...
		$num_entero=$decimales[0];
		$num_decimal=$decimales[1];
		$resultado_final=$num_entero.".".substr($num_decimal,0,1);
		//$resultado_final= number_format($resultado_final, 1, ",", "");  
...
function tipoventa($tipoproducto)
  {
	$respuesta = new xajaxResponse();
	if($tipoproducto=="EM" || $tipoproducto=="OT")
	{
...
function elim_product_orden($guia,$id)
  {
   include("conexion.php"); 
	...
	$result = mysql_query($sql);
	$sql3 ="SELECT  * from detalle_orden_compra where id_orden_compra=".$guia." ";
	$result2 = mysql_query($sql3); 
	$contenido="<tr><th width='34'>ID</th> <th width='296'>Nombre Producto</th> <th width='87'>Cantidad</th> <th width='78'>Valor Unitario</th> <th width='73'>Unidad</th> <th width='56'>Total</th> <th width='80'>Eliminar</th> </tr> ";
	$total=0;
	while($row = mysql_fetch_row($result2))
...
///////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////
function lista_oc($formulario)
  {
	include("conexion.php"); 
    $respuesta = new xajaxResponse();
	$tipo_mezcla = $formulario["tipo_mezcla"];
	$mezclita = $formulario["mezclita"];
	$cantidad = $formulario["cantidad"];
	if($tipo_mezcla=="" || $mezclita ==""  || $cantidad =="" )
	{
		$respuesta->addScript("alert('Datos Vacios, Favor Ingrese Los Datos Faltantes');");
		 return $respuesta;
	}
	$sql3 ="SELECT * from ingreso_orden order by id DESC limit 1";
	$result2 = mysql_query($sql3); 
	while($row = mysql_fetch_row($result2))
		{ 
			$id=$row[0];
	...
	$id=$id+1;	
	$sql="insert into detalle_oc(producto,cantidad,n_oc) values ('$mezclita','$cantidad','$id')";
	$result = mysql_query($sql);
	
	$sql2 ="SELECT * from detalle_oc dp, mezclas ml where dp.n_oc=".$id." and ml.id=dp.producto ";
	$result2 = mysql_query($sql2); 

	$contenido='<tr><th width="67">ID</th><th width="318">Nombre Producto</th><th width="125">Cantidad</th><th width="170">Eliminar</th></tr>';
		while($row = mysql_fetch_row($result2))
		{ 
		$contenido.="<tr>";
		$contenido.="<td>".$row[0]."</td>";
          $contenido.="<td >".$row[6]."</td>";
            $contenido.="<td>".$row[2]."</td>";
            $contenido.="<td><input type='button' onclick='xajax_elim_oc($id,".$row[0].")' value='Eliminar' class='formbutton2' >";
         $contenido.="</tr>";
          }     
    $respuesta->addAssign("listado_productos","innerHTML",$contenido);
 ...
 /////////////////////////////////////////////////////////////////////////////////////////////// 
 /////////////////////////////////////////////////////////////////////////////////////////////// 
function elim_oc($guia,$id)
  {
   include("conexion.php"); 
	$respuesta = new xajaxResponse();
	$sql="delete  from detalle_oc where id='".$id."'";
	$result = mysql_query($sql);
	
	$sql2 ="SELECT * from detalle_oc dp, mezclas ml where dp.n_oc=".$guia." and ml.id=dp.producto ";
	$result2 = mysql_query($sql2); 
	$contenido='<tr><th width="67">ID</th><th width="318">Nombre Producto</th><th width="125">Cantidad</th><th width="170">Eliminar</th></tr>';
		while($row = mysql_fetch_row($result2))
		{ 
		...
		$contenido.="<td>".$row[0]."</td>";
          $contenido.="<td >".$row[6]."</td>";
            $contenido.="<td>".$row[2]."</td>";
            $contenido.="<td><input type='button' onclick='xajax_elim_oc($guia,".$row[0].")' value='Eliminar' class='formbutton2' >";
         $contenido.="</tr>";
          }     
    $respuesta->addAssign("listado_productos","innerHTML",$contenido);
    return $respuesta;
...
$xajax->registerFunction("operacion_calculo");
