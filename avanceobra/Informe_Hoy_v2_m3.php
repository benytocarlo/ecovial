...
$sql = "select * from control_dia where nombre_obra='$nombre_obra' and eq_asfalto!= ' ' and fecha<='$fecha' order by fecha asc";
	//echo $sql;
$result = mysql_query($sql,$conexion) or die(mysql_error());

$sql2 = "select * from control_dia where nombre_obra='$nombre_obra' and (eq_maq_fre!= ' ' or eq_maq_maf!= ' ') and fecha<='$fecha' order by fecha asc ";
	//echo $sql2;
$result2 = mysql_query($sql2,$conexion) or die(mysql_error());

$sql3 = "select * from control_dia where nombre_obra='$nombre_obra' and (imprimacion!= ' ' or riego!= ' ') and fecha<='$fecha' order by fecha asc ";
	//echo $sql3;
...
?>
  ...
  <table width="457" border="1">
    <tr>
      <td width="54"><div align="center"><strong>Fresado</strong></div></td>
      <td width="36"><div align="center"><strong>MAF</strong></div></td>
      <td width="49"><div align="center"><strong>Mezcla</strong></div></td>
      <td width="83"><div align="center"><strong>Imprimación</strong></div></td>
      <td width="121"><div align="center"><strong>Riego de Liga</strong></div></td>
      <?php while($row =mysql_fetch_row($result3))
		{
//		$m3_mez=0;
...
//		echo $m3_mez;
    ...
    <?php while($row =mysql_fetch_row($result2))
		...
		$total_imprimacion=(($suma22*100)/$imprimacion);
		$total_riego=(($suma33*100)/$riego);
...
//		$total_mez=(($suma1*100)/$m_maf);
    ...
    <tr>
       ...
       <?php echo "<td>".number_format($total_maf, 0, '', '.')." %</td>"; ?>
        <?php echo "<td>".number_format($total_mez, 0, '', '.')." %</td>"; ?> 
        <?php echo "<td>".number_format($total_imprimacion, 0, '', '.')." %</td>"; ?> 
    <?php echo "<td>".number_format($total_riego, 0, '', '.')." %</td>"; ?>    </tr>
  </table>
  <p class="texto2">&nbsp;</p>
</div>
<p>&nbsp;</p>
