...
$id=$_REQUEST["id"];
	...
	if($oculto=="camion")
	{
		...
		$sql="update camiones set tipo_vehiculo='$tipo_vehiculo', patente='$patente', tara='$tara', empresa='$empresa', capacidad='$capacidad', num_camion='$num_camion' where id=$id";
	$result = mysql_query($sql);
	...
	if($oculto=="ingreso_materias")
	{
		...
		//$sql2="insert into usuarios(nom_comp,nom_user,pass_user,privilegio) values ('$nombre','$user','$pass','$privilegio')";
		$sql="update ingreso_materias set orden_compra='$orden', guia='$guia', proveedor='$proveedor',transportista='$transportista', materia='$materia' ,fecha_guia='$fecha',cantidad_total='$cantidad',precio_unitario='$precio',patente='$patente',peso_bruto='$peso',tara='$tara',n_pesaje='$n_pesaje',cantidada_calculada='$cant_calculada' where id=$id";
		
		$result = mysql_query($sql);
		echo"<script>alert('Ingreso de Materias Actualizado Exitosamente')</script>";
		echo"<script>location='registro_materias_primas_confirmacion_actualizar.php?id=$id'</script>";
	}
