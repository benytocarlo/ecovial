...
function lista_orden($formulario)
  {
	...
	$result2 = mysql_query($sql2); 
		$contenido="<tr><th width='127'>Nombre Producto</th> <th width='154'>Cantidad</th> <th width='122'>Valor Unitario</th> <th width='87'>Unidad</th> <th width='94'>Total</th> </tr> ";
		while($row = mysql_fetch_row($result2))
		{ 
	...
	*/
	...
	$result2 = mysql_query($sql2); 
		...
		{ 
		$contenido.=" <tr>";
          $contenido.="<td height='58'>".$row[2]." </td>";
            $contenido.=" <td>".$row[0]."</td>";
           $contenido.="  <td> ".$row[3]."</td>";
            $contenido.=" <td>".$row[1]."</td>";
			$total_final=$row[0]*$row[3];
...
function lista_guia($formulario)
  {
	...
	$sql2 ="SELECT * from detalle_pesaje where guia=".$n_guia." ";
	$result2 = mysql_query($sql2); 
...
function calcularm3($orden_compra,$factor_mezcla,$pesobruto,$tara,$orde_compra)
  {
	...
	{
		print ("Ingrese La Orde de Compra");
		exit();
	}else{
